...
import { CatalogHeader, CatalogToolbar, AlgorithmGrid, EmptyState } from "@/components/catalog";
...
export function CategoryPageClient({ algorithms, categoryInfo }: CategoryPageClientProps) {
  // ── URL state ──
  ...
  if (!categoryInfo.isAvailable) {
    return (
      <>
        <CatalogHeader category={categoryInfo} algorithmCount={0} />
        <EmptyState type="empty-category" categoryName={categoryInfo.label} />
      </>
    );
  }

  const isFiltered =
    state.difficulties.size > 0 || state.searchQuery !== "" || state.sortBy !== "name-asc";

  return (
    <>
      <CatalogHeader category={categoryInfo} algorithmCount={totalCount} />
