...
import { matchesSearch, filterAlgorithms, sortAlgorithms } from "@/hooks/useCatalogFilters";
...
describe("sortAlgorithms", () => {
  const algos = [
    ...
    makeMeta({
      name: "Alpha Search",
      complexity: { time: "O(n)", space: "O(1)", level: "advanced" },
    }),
    makeMeta({
      name: "Middle Path",
      complexity: { time: "O(n)", space: "O(1)", level: "intermediate" },
    }),
  ];

  it("sorts by name ascending", () => {
    const sorted = sortAlgorithms(algos, "name-asc");
    expect(sorted.map((a) => a.name)).toEqual(["Alpha Search", "Middle Path", "Zebra Sort"]);
  });

  it("sorts by name descending", () => {
    const sorted = sortAlgorithms(algos, "name-desc");
    expect(sorted.map((a) => a.name)).toEqual(["Zebra Sort", "Middle Path", "Alpha Search"]);
  });

  it("sorts by difficulty ascending", () => {
    const sorted = sortAlgorithms(algos, "difficulty-asc");
    expect(sorted.map((a) => a.complexity.level)).toEqual(["beginner", "intermediate", "advanced"]);
  });

  it("sorts by difficulty descending", () => {
    const sorted = sortAlgorithms(algos, "difficulty-desc");
    expect(sorted.map((a) => a.complexity.level)).toEqual(["advanced", "intermediate", "beginner"]);
  });
...
describe("filterAlgorithms", () => {
  const algos = [
    ...
    makeMeta({
      ...
      complexity: { time: "O(n)", space: "O(n)", level: "advanced" },
    }),
    makeMeta({
      id: "c",
      category: "classical",
